...
class Executor {
    ...
    private void goSudoku() {
        ...
        System.out.format("%.5f seconds to execute", ((end - start) / 1000000000.0));
    }
        ...
        for (int j = startColumn; j < endColumn; j++) {
            if (sudoku[row][j][0][0] == 0) {
                for (int k = 0; k < 9; k++) {
                    if (tempNumbers[k] == 0 && sudoku[row][j][1][k] != 0) {
                        if (debug)
                            System.out.println("(closed row Triple) deleted value " + sudoku[row][j][1][k] + " [" + row + "; " + j+ "]");
                        sudoku[row][j][1][k] = 0;
                        goFurther = true;
                    }
                }
            ...
            if (sudoku[row][column][0][0] == 0) {
                for (int k = 0; k < 9; k++) {
                    if (tempNumbers[k] == 0 && sudoku[row][column][1][k] != 0) {
                        if (debug)
                            System.out.println("(closed row Double) deleted value " + sudoku[row][column][1][k] + " [" + row + "; " + column + "]");
                        sudoku[row][column][1][k] = 0;
                        goFurther = true;
                    }
                }
        ...
        for (int j = startRow; j < endRow; j++) {
            if (sudoku[j][column][0][0] == 0) {
                for (int k = 0; k < 9; k++) {
                    if (tempNumbers[k] == 0 && sudoku[j][column][1][k] != 0) {
                        if (debug)
                            System.out.println("(closed column Triple) deleted value " + sudoku[j][column][1][k] + " [" + j + "; " + column + "]");
        ...
        for (int row : rowsWithSuitableValues) {
            if (sudoku[row][column][0][0] == 0) {
                for (int k = 0; k < 9; k++) {
                    if (tempNumbers[k] == 0 && sudoku[row][column][1][k] != 0) {
                        if (debug)
                            System.out.println("(closed column Double) deleted value " + sudoku[row][column][1][k] + " [" + row + "; " + column + "]");
                        sudoku[row][column][1][k] = 0;
                        goFurther = true;
                    }
                }
